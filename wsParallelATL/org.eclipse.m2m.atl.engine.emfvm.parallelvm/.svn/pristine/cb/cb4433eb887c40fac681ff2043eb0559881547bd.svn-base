...
public class ASMTXMLReader extends ASMXMLReader{
	
	private ASMT ret = new ASMT();

	private ASMTOperation currentOperation;
	...
	private List<TBytecode> bytecodes;
	...
	public void startElement(String uri, String localName, String qName, Attributes attributes)
		...
		if (qName.equals("asm")) { //$NON-NLS-1$
			...
			currentOperation = new ASMTOperation(ret, resolve(attrs.get("name"))); //$NON-NLS-1$
			bytecodes = new ArrayList<TBytecode>();
			...
			if (inCode) {
				if (attrs.containsKey("arg")) { //$NON-NLS-1$
					...
					// } else {
					bytecodes.add(new TBytecode(qName, resolve(attrs.get("arg")))); //$NON-NLS-1$
					// }
				} else {
					bytecodes.add(new TBytecode(qName));
				}
			}
		}
	}
	...
	public void endElement(String uri, String localName, String qName) throws SAXException {
		if (qName.equals("cp")) { //$NON-NLS-1$
			...
			currentOperation.setBytecodes(bytecodes.toArray(new TBytecode[0]));
